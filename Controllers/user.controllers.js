...
const getSearchResults = asyncWrapper(async (req, res) => {
  const data = await Content.find(
    ...
    .sort({ score: { $meta: "textScore" } })
    .limit(20);
  res.render("user/searchDashboardV2", {
    page: "search",
    data,
    resultdescription: `${req.query.search}`,
    isAllUsers: false,
    isAllMessages: false,
    isFreeTime: false,
    isAnalytics: false,
...
const getPlayPremiumVideo = asyncWrapper(async (req, res) => {
  ...
  data.videoUrl = url;
  let similarVideos = await Content.find({ topic: data.topic }).sort({
    lessonNumber: 1,
  });
  ...
  if (search) {
    ...
    if (username === "0775527077" && email === "twaninetschool@gmail.com") {
      res.render("user/adminDashboard", {
        ...
        resultdescription: `${search}`,
        page: "dashboard",
      });
    } else {
      res.render("user/dashboardV2", {
        data,
        level: "dummy",
        subject: "english",
        activeMenuItem: "dashboard",
...
const getUserSubject = asyncWrapper(async (req, res) => {
  ...
  res.render("user/dashboardV2Class", {
...
const getUserLevel = asyncWrapper(async (req, res) => {
  ...
  res.render("user/dashboardV2Term", {
    subjectSelected: subject,
    levelSelected: level,
    schoolTerms,
    page: "dashboard",
...
const getUserTerm = asyncWrapper(async (req, res) => {
  ...
  res.render("user/dashboardV2Topic", {
    subjectSelected: subject,
    levelSelected: level,
    termSelected: term,
    topics,
...
const getUserTopic = asyncWrapper(async (req, res) => {
  ...
  res.render("user/dashboardV2Lesson", {
    subject,
    level,
    term,
    topic,
